...
int main(void) {
  ...
  MX_TIM5_Init();
  MX_USB_DEVICE_Init();
  /* Perform 9-clock I2C bus recovery before initializing hardware I2C3 */
  i2c_bus_recovery_perform();
  MX_I2C3_Init();

  /* USER CODE BEGIN 2 */
  /* 1. Initialize hardware encoders (TIM4, TIM3, TIM2) */
  hw_encoders_init(&htim4, &htim3, &htim2);

  /* 2. Initialize 4x MD30C Motor Drivers (PWM on TIM5 CH1..CH4, DIR on GPIOB)
   */
  motor_channel_config_t my_motors[NUM_MOTORS] = {
  ...
  motor_driver_init(&htim5, my_motors);

  /* 3. Initialize Level 3 & 4 Tasks, Queues, Semaphores, Mutexes, and Software
   * Timer */
  task_manager_init();

  /* 4. Start FreeRTOS Preemptive Real-Time Scheduler */
  vTaskStartScheduler();
  /* USER CODE END 2 */

  /* Infinite loop (only reached if scheduler fails to start) */
